...
2. Run the command `pod install` to install the dependencies inside the project root folder.
   > **Note:** `pod install` must be run before opening the project. The `TelnyxRTC.xcworkspace` file is **generated by CocoaPods** and is gitignored — it will not exist until you run `pod install`.
3. Open the Workspace : `TelnyxRTC.xcworkspace`
   > **Important:** Always open `TelnyxRTC.xcworkspace` (not `TelnyxRTC.xcodeproj`). Opening the `.xcodeproj` directly will fail to resolve dependencies.
...
> ⚠️ **CallKit on the iOS Simulator:** CallKit cannot be fully validated on the iOS Simulator. Incoming calls and the CallKit UI (incoming call screens, call notifications) must be tested on a **physical device**. The Simulator does not reliably deliver VoIP pushes or render CallKit interfaces.
