...
- **Reconnect Timeout Calls-Dictionary Race (VSDK-336 / IOS-C20)**: `TxClient.startReconnectTimeout()`'s timer event handler now dispatches its `self.calls` work to the main queue instead of executing it on `reconnectQueue` directly. Previously the timer fired on the private `reconnectQueue` and immediately iterated `self.calls` (`updateActiveCallsState()` + `disconnect()`); while the main thread was concurrently mutating `self.calls` from `newCall`, `endFromCallkit`, `createIncomingCall`, or `callStateUpdated`, Swift's Dictionary would trap with `EXC_BAD_INSTRUCTION` ("mutated during iteration"). All `self.calls` reads and writes are now serialized on the main thread, matching the existing inline comment that already documented this intent.
